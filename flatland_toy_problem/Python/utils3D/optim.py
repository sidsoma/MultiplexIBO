...
class Model(nn.Module):
    ...
    """
    def __init__(self, numPixels_z, numPixels_y, numSpots):
        super().__init__()
        # initialize weights with random numbers
        vis = torch.rand(numPixels_z, numPixels_y, numSpots)
...
#         self.debug_v.retain_grad()
        obs = torch.sum(hists * vis.unsqueeze(-1), axis=2)
        return obs
...
def training_loop(model, histograms, observations, optimizer, thresh, lam, n=1000):
    ...
    for i in range(n):
        ...
        loss2 = torch.sum(torch.square(torch.diff(model.vis, n=1, axis=0))) + torch.sum(torch.square(torch.diff(model.vis, n=1, axis=1))) + torch.sum(torch.square(torch.diff(model.vis, n=1, axis=2)))
